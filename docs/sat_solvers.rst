...
There are already solvers, encodings and planners in the library, but you may be interested in creating your own classes. This tutorial explains how to implement any of those. Please note, however, that it is possible to implement only one or two of those, and use the default implementation of the other. However, if you create a new encoding OR if you add a solver, then you MUST also create a new planner.

  #. Create a simple Java project with PDDL4J
  #. Create the main class of our encoding (optional)
  #. Add a SAT solver (optional)
  #. Create the main class of our planner (optional)
  #. Run our example
...
Step 2. Create the main class of our encoding (optional)
--------------------------------------------
...
Finally, ``encodeFrameAxioms``, which takes as argument the considered state (between `0` and `planLength - 1`), is here calling two other methods:
...
Step 3. Add a SAT solver (optional)
--------------------------------------------
...
If your solver implements ipasir, your aim is now to get a ``.so`` file (shared object) for your solver. If you are lucky, your solver has a makefile with an option to build a ``.so`` file; otherwise, you will probably need to use ``gcc`` or modify your solver's makefile to get it (which is not the subject of this tutorial).

For the sake of this example, we will choose `mergesat <https://github.com/conp-solutions/mergesat>`_. In this case (at least at the time when this tutorial is written), running ``make all`` is enough to generate a file ``libmergesat.so.2.1.0`` (which we'll need to rename ``libmergesat.so``, because the file has to end with ``.so`` for the following to work correctly).

Move this .so file into the ``lib`` directory in your project.

You now have to create a simple wrapper in Java in order to be able to use this solver from your Java code. **Create and edit a file** ``MergesatWrapper.java`` **in the directory** ``src/fr/uga/pddl4j/examples/sat``. The skeleton of this class is given below:
...
Please make sure that the path (from the root of your project) specified as argument of the call to ``Native.load`` is correct (it should be if you followed the instructions).

At this point, you've managed to add a solver to your project. However, in order to use it, you need to create a new SAT planner, as described in the next part.

Step 4. Create the main class of our planner (optional)
--------------------------------------------
...
There are two methods that you may have to override: ``getSATEncodingFromName(String encodingName)`` (necessary only if you added an encoding) and ``getSATSolverFromName(String solverName)`` (necessary only if you added a solver).
...
Don't forget to call the parent methods at the end of each of the two methods if you want to be able to also use the encodings and solvers provided with the library.
...
Whether or not you have added a solver or created a SAT planner or a SAT encoding, you may use the provided classes this way:
...
Or, if you want to specify the solver (``-s PICOSAT``, for example), the encoding (``-e DEFAULT``), the maximum plan length (``-mpl 1000``) and the timeout in seconds (``-t 10``), you may also specify them like this:
...
If you have followed all the parts of this tutorial, you can of course use your new planner with your solver and your encoding. First, compile your code:

.. code-block:: bash

    javac -d classes -cp lib/pddl4j-4.0.0.jar src/fr/uga/pddl4j/examples/sat/*.java

Then, you can run it the same way, by simply replacing ``fr.uga.pddl4j.planners.sat.GenericSATPlanner`` by your own planner, that is:
...
Naturally, you may also use provided solvers (e.g. ``PICOSAT``) and/or provided encodings (e.g. ``DEFAULT``) with your new planner. Please note that the reverse is false: you *can't* use your new encoding or your new solver with the provided ``GenericSATPlanner``.
