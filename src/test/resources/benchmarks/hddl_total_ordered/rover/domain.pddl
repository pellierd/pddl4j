(define (domain rover)
  ...
  (:tasks
    (calibrate_abs ?rover - rover ?camera - camera)
    (empty-store ?s - store ?rover - rover)
    (get_image_data ?objective - objective ?mode - mode)
    (get_rock_data ?waypoint - waypoint)
    (get_soil_data ?waypoint - waypoint)
    (navigate_abs ?rover - rover ?to - waypoint)
    (send_image_data ?rover - rover ?objective - objective ?mode - mode)
    (send_rock_data ?rover - rover ?waypoint - waypoint)
    (send_soil_data ?rover - rover ?waypoint - waypoint)
   )
