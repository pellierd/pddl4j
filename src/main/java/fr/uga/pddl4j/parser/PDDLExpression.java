...
public class PDDLExpression extends AbstractExpression<Symbol<String>, PDDLTypedSymbol> implements ParsedObject {
    ...
    public PDDLExpression(final PDDLExpression other) {
        ...
        if (other.getChildren() != null) {
            ...
            for (int i = 0; i < other.getChildren().size(); i++) {
                this.addChild((PDDLExpression) other.getChildren().get(i));
            }
        }
        this.parsedObject = new AbstractParsedObject(other.parsedObject);
    }
        ...
        switch (this.getConnective()) {
            ...
            default:
                for (int i = 0; i < this.getChildren().size(); i++) {
                    PDDLExpression ei = (PDDLExpression)  this.getChildren().get(i);
    ...
    public Set<Symbol<String>> getTaskIDs() throws UnexpectedExpressionException {
        ...
        switch (this.getConnective()) {
            ...
            case HOLD_DURING_METHOD_CONSTRAINT: // Add method ordering HDDL2.1
                for (int i = 0; i < this.getChildren().size(); i++) {
                    final PDDLExpression exp = (PDDLExpression) this.getChildren().get(i);
    ...
    @Override
    public PDDLExpression getInstance(PDDLConnective connective) {
        return new PDDLExpression(connective);
    }
