        ...
        for (PDDLTypedSymbol elt : list) {
            ...
            if (types.size() > 1) {
                ...
                for (PDDLSymbol type : types) {
                    ...
                    buf.append("~");
                    buf.append(image);
                    int typeIndex = Encoder.pb.getTableOfTypes().indexOf(image);
                    final Set<Integer> typeDomain = Encoder.pb.getTableOfDomains().get(typeIndex);
                    newTypeDomain.addAll(typeDomain);
                }
                newType = buf.toString();
                int index = Encoder.pb.getTableOfTypes().indexOf(newType);
                if (index == -1) {
                    Encoder.pb.getTableOfDomains().add(new LinkedHashSet<>(newTypeDomain));
                    Encoder.pb.getTableOfTypes().add(newType);
                }
            }
        }
    }
