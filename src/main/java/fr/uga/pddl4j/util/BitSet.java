...
/**
 * This class implements a vector of bits that grows as needed. This class is based on the same code as the
 ...
 *
 * <p>Every bit set has a current size, which is the number of bits of space currently in use by the bit set. Note that
 * the size is related to the implementation of a bit set, so it may change with implementation. The length of a bit set
 * relates to logical length of a bit set and is defined independently of implementation.
 *
 * <p>Unless otherwise noted, passing a null parameter to any of the methods in a {@code BitSet} will result in a
 * {@code NullPointerException}.
 *
 * <p>A {@code BitSet} is not safe for multithreaded use without external synchronization.
 *
 * <p>This class is a import a {@code BitSet} in
 *
...
public class BitSet implements Cloneable, java.io.Serializable {
    ...
    /**
     * The bits in this BitSet.  The ith bit is stored in bits[i/64] at bit position i % 64 (where bit position 0 refers
    ...
    /**
     * Change to protected to access to the attribute in subclasses.
     ...
     *
     * <p>The byte buffer is not modified by this method, and no reference to the buffer is retained by the bit set.
     *
     * @param bb a byte buffer containing a little-endian representation of a sequence of bits between its position and
     ...
     */
    protected void ensureCapacity(int wordsRequired) {
    ...
    /**
     * Sets each bit from the specified {@code fromIndex} (inclusive) to the specified {@code toIndex} (exclusive) to
    ...
    /**
     * Returns true if the specified {@code BitSet} has any bits set to {@code true} that are also set to {@code true}
     * in this {@code BitSet}.
     *
     ...
     * returned in order, from lowest to highest. The size of the stream is the number of bits in the set state, equal
     * to the value returned by the {@link #cardinality()} method.
     *
     * <p>The bit set must remain constant during the execution of the terminal stream operation. Otherwise, the result
     * of the terminal stream operation is undefined.
     *
